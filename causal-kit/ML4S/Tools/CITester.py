...
class CITester(object):
    def __init__(self, indexedDataT, confidenceLevel=0.99, maxConSizeForBackSearch=2, maxCountOfSepsets=50,
                 maxPairsCount=100, maxEstimandCondCount=1000):
    ...
    def ConditionalIndependenceTest(self, x, y, S=None):
        ...
        if not S:
            ...
            # pValue, severity = IndependentTest.RobustCITest(self.DataT[inds], self.Cardinalities[inds])
    ...
    def GetSeparatingSetsFromPC(self, X, Y, PCX, PCY):
        ...
        search_from_pcy = set.union(*[set(combinations(PCY, condsize)) for condsize in
                                      range(1, 1 + min(self.maxConSizeForBackSearch,
                                                       len(PCY)))])  # condsize from 1, bcs empty set already tested.
    ...
    def ExtractTForkFeatureBasedOnPC(self, T, X, Y, PCT, PCX, PCY, Xpcypairs, Ypcxpairs, PCX_PCY_pairs):
        ...
        XY_sepsets = list(
            self.GetSeparatingSetsFromPC(X, Y, PCX, PCY))  # should it be [set()] if the maxpval still <= 0.01
        ...
        return scalings + overlaps + [[_condon(ests, conds) for ests in estimands_catagories] for conds in
                                      conditions_categories]
