 ...
 */
public class InAsobsSearcher extends AsobsSearcher {

    private int[][] cand;

    private int[] todo;

    public InAsobsSearcher(BaseSolver solver) {
        super(solver);
    }
    ...
    public ParentSet[] search() {
        ...
        while (solver.still_time) {
            ...
            // Random swap
            if (cnt < n_var - 1) {
                int index = solver.randInt(cnt + 1, n_var - 1);

                ArrayUtils.swap(todo, cnt, index);
            }

            if (greedy(vars, todo[cnt])) {
                cnt = 0;
            } else {
                cnt++;
            }
