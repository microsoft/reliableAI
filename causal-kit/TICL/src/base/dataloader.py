...
class Dataset(object):
    def __init__(self, data_path, downsample=None):
        """
        note: we calculate unique at this data prep step, so that each variable's observations are inversed indexs 0, 1, ... 
            and so no need to unique again and again during CITest. (In fact, the benchmarks .txt/.npy is already saved as inversed indexes.)
        Here is example:
            raw_data = np.array([[1, 3, 5],
                                 [2, 2, 3],
                                 [3, 3, 4],
                                 [2, 4, 5]])
            
            IndexedDataT = array([[0, 1, 2, 1],
                                  [1, 0, 1, 2],
                                  [2, 0, 1, 2]])
        ...
        """
        raw_data = np.loadtxt(data_path) if data_path.endswith('.txt') else np.load(data_path)  # shape is [sampleSize, varCount]
        ...
        raw_data = raw_data.T  # shape is [varCount, sampleSize]

        def _unique(row):
            return np.unique(row, return_inverse=True)[1]
        
        self.IndexedDataT = np.apply_along_axis(_unique, 1, raw_data).astype(np.int32)
        self.VarCount, self.SampleSize = self.IndexedDataT.shape[0], self.IndexedDataT.shape[1]
