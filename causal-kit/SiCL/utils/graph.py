...
class DiGraph(object):
    ...
    def detectCycles(self):
        ...
        Data: 2023.01.19
        """
        directedG = nx.DiGraph(self.adjmat)
        # directiG = ig.Graph.Adjacency(adjmat)
        return list(nx.simple_cycles(directedG))


class MixedGraph(object):
    def __init__(self, numberOfNodes=0, nodeIDs=None, adjmat=None):
        if adjmat is None:
    ...
    def apply_meek_rules(self):
        # preCycles = len(self.detectCycles())
        ...
        def _apply_rule(y, meek_rule_id):
            ...
            for x in y_neighbors:
                if meek_rule(x, y):
                    ...
                    # if len(self.detectCycles()) > preCycles:
                    #     self.del_di_edge(x, y)
                    #     self.add_undi_edge(x, y)
                    # else:
                    #     stack.append(y)
                    stack.append(x)
                elif meek_rule(y, x):
                    ...
                    # if len(self.detectCycles()) > preCycles:
                    #     self.del_di_edge(y, x)
                    #     self.add_undi_edge(y, x)
                    # else:
                    #     stack.append(x)
                    stack.append(x)
    ...
    def detectCycles(self):
        ...
        Data: 2023.11.21
        """
        adjmat = np.zeros((len(self.NodeIDs), len(self.NodeIDs)), dtype=int)  # keep directed edges
        if self.DirectedEdges:
            di_inds = tuple(np.array(list(self.DirectedEdges)).T)
            adjmat[di_inds] = 1
        directedG = nx.DiGraph(adjmat)
        # directiG = ig.Graph.Adjacency(adjmat)
        return list(nx.simple_cycles(directedG))
