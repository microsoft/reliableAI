...
from avici_Tools.synthetic import Data
from avici_Tools.utils.graph import mat_to_toporder
...
def sample_recursive_scm(*,
    ...
    for interv_target in [None] + list(range(n_observations_int // multiple_interv)):
        ...
        if interv_target is None:
            ...
            data_type = "int"
            if multiple_interv:
                num_intervened = random.randint(1, 5)
                # num_intervened = 1
                index_intervened = set(random.sample(range(n_vars), k=num_intervened))
                is_intervened = [True if i in index_intervened else False for i in range(n_vars)]
            else:
                is_intervened = onp.eye(n_vars)[interv_target].astype(bool)
            # n_obs = math.ceil(n_observations_int / n_interv_vars)
            n_obs = multiple_interv
        ...
        for j in toporder:
            ...
            if is_intervened[j]:
                x[:, j] = interv_dist(rng, shape=z_j.shape) # + 1.0 * f[j](x=x, z=z_j, is_parent=g[:, j])
            else:
    ...
    if simulate_interv_data:
        x_int = onp.stack(data["int"]["x"]).reshape(-1, n_vars)
        x_int_msk = onp.stack(data["int"]["interv_mask"]).reshape(-1, n_vars)
    else:
