...
import avici_Tools
from avici_Tools.definitions import YAML_CLASS, YAML_MODULES, REAL_DATASET_KEY
from avici_Tools.synthetic import SyntheticSpec, CustomClassWrapper
from avici_Tools.utils.load import load_yaml
...
def _parse_config_tree(config, force_kwargs=None, module_paths=None):
    ...
    if type(config) in [dict, defaultdict]:
        ...
        if is_class:
            ...
            try:
                cls = getattr(avici_Tools.synthetic, name)
                new = cls(**kwargs)
