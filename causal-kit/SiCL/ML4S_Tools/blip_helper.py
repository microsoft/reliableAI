...
root_path = "/home/v-jiaruzhang/transformer_scl"
...
def parent_set_iden(tmp: tempfile.TemporaryDirectory):
    ...
    dat_path = os.path.join(tmp.name, f"data.dat")
    jkl_path = os.path.join(tmp.name, f"parent_set.jkl")

    os.system(f"java -Xmx200G -jar {root_path}/blip/lib/blip.jar scorer.is -d {dat_path} -j {jkl_path} -t {TIMEOUT1} -b 0")
...
def general_struc_opt(tmp: tempfile.TemporaryDirectory, res_path: str):
    ...
    print(f"java -Xmx200G -jar {root_path}/blip/lib/blip.jar solver.winasobs.adv -smp ent  -d {dat_path} -j {jkl_path} -r {res_path} -t {TIMEOUT2} -b 0")
    os.system(f"java -Xmx200G -jar {root_path}/blip/lib/blip.jar solver.winasobs.adv -smp ent  -d {dat_path} -j {jkl_path} -r {res_path} -t {TIMEOUT2} -b 0")
...
if __name__ == "__main__":
    dataset_name = "cancer"
    dataset_path = f"{root_path}/ML4S_Experiments/benchmarks/npy10000/{dataset_name}.npy"
    data = np.load(dataset_path)
    dag = run_blip(pd.DataFrame(data), f"{root_path}/blip/data/{dataset_name}.res")
    # dag = run_blip(pd.DataFrame({"a": [1, 2, 3], "b": [1, 2, 3]}), f"{root_path}/blip/data/{dataset_name}.res")
    print(dag)