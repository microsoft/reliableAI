...
class DiscreteDataset(object):
    def __init__(self, data, downsample=None):
        """
        ...
        if isinstance(data, str):
            raw_data = np.loadtxt(data) if data.endswith('.txt') else np.load(data)  # now (sampleSize, varCount)
        else:
            raw_data = data
        ...
        self.IndexedDataT = raw_data.astype(np.int32)
...
class ContinousDataset(object):
    def __init__(self, data, downsample=None):
        ...
        # self.IndexedDataT = np.apply_along_axis(_unique, 1, raw_data).astype(np.int32)
        self.IndexedDataT = raw_data.astype(np.float32)
        self.SampleSize = self.IndexedDataT.shape[1]
        self.VarCount = self.IndexedDataT.shape[0]
