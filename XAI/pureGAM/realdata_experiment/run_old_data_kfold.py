...
if __name__ == "__main__":
    ...
    print(df)
    X_num = df.loc[:, ["Age", "Civil", "Can/US"]].values
    ...
    X_num = df.loc[:, ["in1", "in2", "in3", "in4"]].values
    X_cate = df.loc[:, []].values
    # "L3","L4","L5"
    y = df.loc[:, ["out1"]].values
    run_pureGam_gami_kfold(X_num, X_cate, y, base_results_folder, seed=seed)
    ...
    X_num = df.loc[:,
            ["subject.", "age", "sex", "test_time", "motor_UPDRS", "total_UPDRS", "Jitter...", "Jitter.Abs.",
             ...
             "Shimmer.APQ11", "Shimmer.DDA", "NHR", "HNR", "RPDE", "DFA"]].values
    X_cate = df.loc[:, []].values
    # "L3","L4","L5"
    y = df.loc[:, ["PPE"]].values
    #y = power_transform(y)
