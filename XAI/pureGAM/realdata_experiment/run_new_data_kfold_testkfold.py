...
from synthetic_experiments.run_metrics import predict_vec_pureGAM, score_pureGAM, score_pureGAM_cat, score_pureGAM2, score_gami2
...
def run_pureGam_gami_kfold(X_num, X_cate, y, results_folder, isPureScore=True, seed=42):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(X_num)):
        ...
        #todo: no run pureGAM
        pureGam_hmap = run_pureGam(X_num_train, X_cate_train, y_train, X_num_test, X_cate_test, y_test,
...
def run_pureGam(X_num, X_cate, y_train, X_num_test, X_cate_test, y_test, results_folder, isPureScore=True):
    ...
    try:
        ...
        t0 = time.time()
        model.train(train_loader, valid_loader, optimizer=optimizer, num_epoch=2 if isTest else int(10*200 * math.sqrt(1e4 / len(y))),
                    tolerent_level=int(20 * math.sqrt(1e4 / len(y))),
    ...
    int_list = model.pairwise_idxes_num
    ...
    ret_pureGam_hmap = model.num_enc.get_lam()[0].detach().cpu().numpy().tolist()
    if isPureScore:
        pred_int = model.predict_biv_contri_mat(train_loader_all)

        #todo: make sure clear memory for
        del model, train_loader_all, test_loader, train_loader, valid_loader
...
def run_gami(X_num, X_cate, y_train, X_num_test, X_cate_test, y_test, task_type, meta_info, results_folder, h_map, isPureScore=True):
    ...
    model = GAMINet(meta_info=meta_info, interact_num=X_num.shape[1],
                    ...
                    main_effect_epochs=int(2*500 * math.sqrt(1e4 / X_num.shape[0])),
                    interaction_epochs=int(2*500 * math.sqrt(1e4 / X_num.shape[0])),
